...
def main():
    parser = argparse.ArgumentParser(
        description="Convert desklib/ai-text-detector-v1.01 (or another HF model) to ONNX using torch.onnx.export."
    )
    parser.add_argument(
        "--model-id",
        type=str,
        default="desklib/ai-text-detector-v1.01",
        help="Hugging Face model ID (default: desklib/ai-text-detector-v1.01)",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="models/desklib-ai-text-detector-v1.01-onnx",
    ...
    print("  - model.onnx")
    print("  - config.json")
    print("  - tokenizer.json / tokenizer_config.json / special_tokens_map.json / spm.model (SentencePiece)")
